...
##just for clarity of thought (as these are redundant)
* y5: prob that a loan will be repaid as per originally agreed timeline (which is 1-y1-y3)
* y6: the timing for paying back as agreed is 1 (by construction).
...
#alterntate version with no constructed vars
y_vars = ['probDef','probER', 'fracNumPmts']
...
model = customNN(emb_dims=emb_dims, num_float_x=len(float_x_vars),
	 lin_layer_sizes = [50,50], output_size=len(y_vars),
...
colListSimple = ['probDefPred','probERPred', 'fracNumPmtsPred']+['probDef','probER', 'fracNumPmts']

#pdf = pd.DataFrame(torch.cat([preds_test,y_test],1).detach().numpy(), columns=colList)
pdf = pd.DataFrame(torch.cat([preds_test,y_test],1).detach().numpy(), columns=colListSimple)

#save to csv
#pdf.to_csv('data/testNNresult.csv', index=False)
pdf.to_csv('data/testNNresultSimple.csv', index=False)
